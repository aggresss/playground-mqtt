...
int main( void )
{
     ...
     */
    ret = mbedtls_x509_crt_parse_file( &srvcert, CERT_PATH"server.crt" );
    if( ret != 0 )
    {
    ...
    ret = mbedtls_x509_crt_parse_file( &srvcert, CERT_PATH"ca.crt" );
    if( ret != 0 )
    {
        mbedtls_printf( " failed\n  !  mbedtls_x509_crt_parse returned %d\n\n", ret );
        goto exit;
    ...
    ret =  mbedtls_pk_parse_keyfile( &pkey, CERT_PATH"server.key", NULL);
    if( ret != 0 )
    {
