...
static int tcpConnect(const char* svr, short port)
{
    ...
    if (r < 0 && (errno == EWOULDBLOCK || errno == EAGAIN)) {
        ...
        check1(pfd.revents == POLLOUT, "poll return error events: %d",
                pfd.revents);
    }
...
static void sslRead(SSLCon* con)
{
    ...
    while (rd < sizeof(buf) && r) {
        ...
        check1(r >= 0, "SSL_read error return %d errno %d msg %s", r, errno,
                strerror(errno));
        log("read %d bytes\n", r);
        rd += r;
    }
...
static void sslWrite(SSLCon* con, const char *text)
{
    ...
    check1(wd == len, "SSL_write error. return %d errno %d msg %s", wd, errno,
            strerror(errno));
