...
int main(int argc, char **argv)
{
    ...
    /*加载客户端证书*/
    if (SSL_CTX_use_certificate_file(ctx, CERT_PATH"client.crt", SSL_FILETYPE_PEM) != 1) {
        SSL_CTX_free(ctx);
        printf("Failed to load client certificate from %s", CERT_PATH"client.crt");
    }
    /*加载客户端私钥*/
    if (SSL_CTX_use_PrivateKey_file(ctx, CERT_PATH"client.key", SSL_FILETYPE_PEM) != 1) {
        SSL_CTX_free(ctx);
        printf("Failed to load client private key from %s", CERT_PATH"client.key");
    }
    /*验证私钥*/
