...
static int https_connect(HTTP_INFO *hi, char *host, char *port)
{
    ...
    if(https == 1)
    {
        ...
        ret = mbedtls_x509_crt_parse(&hi->tls.cacert, (uint8_t *)ca_crt_rsa_1, sizeof(ca_crt_rsa_1));
        ret = mbedtls_x509_crt_parse(&hi->tls.cacert, (uint8_t *)ca_crt_rsa_2, sizeof(ca_crt_rsa_2));
        if( ret != 0 )
        {
            return ret;
        }
